...
static int _rbt_lookup_helper_serial(rbt_t *rbt, char *key)
{
	int ret = 0;
...
static int _rbt_lookup_helper(rbt_t *rbt, htm_fg_tdata_t *tdata, char *key)
{
	int dir, found;
	rbt_node_t *curr;
	unsigned long long window_versions[1]; /* curr version */
	int retries = -1;
	int window_retries = -1;
	unsigned status;

#	ifdef USE_CPU_LOCK
	int tid = tdata->tid;
	int my_cpu_lock = tid % 4;
#	endif
...
#	ifdef USE_CPU_LOCK
	while (cpu_locks[my_cpu_lock].owner > 0 &&
	       ...
	       cpu_locks[my_cpu_lock].spinlock == 0)
		;
#	endif

	while (rbt->spinlock == 0)
		;
...
#		endif
		...
		if (rbt->spinlock == 0)
			_xabort(IMPLICIT_ABORT_GLOBAL_LOCK_TAKEN);
...
//		     (_XABORT_CODE(status) == IMPLICIT_ABORT_GLOBAL_LOCK_TAKEN)) {
		     (_XABORT_CODE(status) == IMPLICIT_ABORT_VERSION_ERROR)) {
	...
	/* Walk down the tree. */
	while (1) {
		window_retries = -1; /* Reset the retries for the next window. */
...
#			endif
			...
			if (window_versions[0] != GET_VERSION(curr))
				_xabort(IMPLICIT_ABORT_VERSION_ERROR);

			/* External node reached. */
			if (IS_EXTERNAL_NODE(curr)) {
//				found = (curr->key == key);
				int cmp = strncmp(curr->key, key, MAX_STR_LEN);
				found = (cmp == 0);
				_xend();
				return found;
			}
...
//	if (curr->key == nodes[0]->key)
	if (strncmp(curr->key, nodes[0]->key, MAX_PATH_LEN) == 0)
		return 0;
...
//		dir_from_parent = parent->key < key;
		...
		if (IS_RED(sibling)) {
			...
			if (stack_versions[top] != GET_VERSION(gparent))
				_xabort(IMPLICIT_ABORT_VERSION_ERROR - 6);
...
static inline int _rbt_delete_helper_serial(rbt_t *rbt, char *key, 
...
//		int dir = curr->key < key;
	...
	/* Key not in the tree. */
//	if (curr->key != key)
	if (strncmp(curr->key, key, MAX_PATH_LEN) != 0)
		return 0;
...
char *rbt_name()
{
	return "links_bu_external_fg_htm_string";
}
