...
static int _delete(rbt_t *rbt, int key,
                   rbt_node_t *node_stack[MAX_HEIGHT], int *stack_top,
                   color_t *deleted_node_color, int *succ_key,
                   int *original_node_stack_index,
                   rbt_node_t **tree_cp_root, rbt_node_t **conn_point,
                   tdata_t *tdata)
{
	rbt_node_t *leaf, *parent, *leaf_left, *leaf_right;

	*original_node_stack_index = *stack_top;
	...
	leaf_left = leaf->left; leaf_right = leaf->right;
	...
	if (*stack_top == 0) {
		*conn_point = NULL;
		*tree_cp_root = leaf_left;
		if (!(*tree_cp_root)) *tree_cp_root = leaf_right;
	} else {
		...
		*tree_cp_root = leaf_left;
		if (!(*tree_cp_root)) {
			if (*original_node_stack_index == *stack_top) {
				*tree_cp_root = leaf_right;
			} else {
				*tree_cp_root = leaf->right;
			}
		}
	}
	...
	node_stack[*stack_top] = leaf_left;
	if (!leaf_left) {
		if (*original_node_stack_index == *stack_top) {
			node_stack[*stack_top] = leaf_right;
		} else {
			node_stack[*stack_top] = leaf->right;
		}
	}
...
static int _delete_rebalance(rbt_t *rbt, int key, 
                              rbt_node_t *node_stack[MAX_HEIGHT], int stack_top,
                              color_t deleted_node_color,
                              int original_node_stack_index, int succ_key,
                              rbt_node_t **tree_cp_root, rbt_node_t **conn_point,
                              tdata_t *tdata)
{
...
try_from_scratch:
	...
	if (++retries >= TX_NUM_RETRIES) {
		...
		int ret = _delete(rbt, key, node_stack, &stack_top, &deleted_node_color,
		                  &succ_key, &original_node_stack_index,
		                  &tree_cp_root, &connection_point, tdata);
		if (ret == 0) {
			pthread_spin_unlock(&rbt->rbt_lock);
			return 0;
		}
		ret = _delete_rebalance(rbt, succ_key, node_stack, stack_top,
		                  deleted_node_color,
		                  original_node_stack_index, succ_key,
	...
	// Delete and Rebalance using copies
	int ret = _delete(rbt, key, node_stack, &stack_top, &deleted_node_color,
	                  &succ_key, &original_node_stack_index,
	                  &tree_cp_root, &connection_point, tdata);
	if (ret == 0) return 0;
