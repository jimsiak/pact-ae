...
typedef struct {
	...
	long long unsigned tx_starts, tx_aborts, lacqs,
	                   tx_aborts_lookup, tx_aborts_validation;
} tdata_t;
...
{
	printf("TID %3d: %llu %llu ( %llu ) [ %llu %llu ]\n", 
	       tdata->tid, tdata->tx_starts, tdata->tx_aborts, tdata->lacqs,
	       tdata->tx_aborts_lookup, tdata->tx_aborts_validation);
}
...
static inline void tdata_add(tdata_t *d1, tdata_t *d2, tdata_t *dst)
{
	...
	dst->tx_aborts_lookup = d1->tx_aborts_lookup + d2->tx_aborts_lookup;
	dst->tx_aborts_validation = d1->tx_aborts_validation + d2->tx_aborts_validation;
}
...
//	char padding0[CACHE_LINE_SIZE - 2 * sizeof(int) - sizeof(void *) -
//	             2 * sizeof(struct rbt_node *)];
//
//	struct rbt_node *parent, *prev, *succ;
//	color_t color;
//	int live;
//	char padding1[CACHE_LINE_SIZE - 3 * sizeof(struct rbt_node *) -
//	             sizeof(color_t) - sizeof(int)];
...
//	char padding[2*CACHE_LINE_SIZE - sizeof(color_t) - 2 * sizeof(int) -
//	             sizeof(void *) - 5 * sizeof(struct rbt_node *)];
//} __attribute__((packed))  __attribute__((aligned(CACHE_LINE_SIZE))) rbt_node_t;
...
/* operation is 0, 1, 2 for lookup, insertion and deletion respectively. */
static inline void _lookup_verify(rbt_node_t *node, int key, int operation)
{
	if (!node || !node->live)
	...
	if (key < node->key) {
		if (operation == 1 && !IS_SENTINEL_NODE(node->left))
			TX_ABORT(ABORT_VALIDATION_FAILURE);
		...
		if (operation == 1 && !IS_SENTINEL_NODE(node->right))
			TX_ABORT(ABORT_VALIDATION_FAILURE);
...
try_from_scratch:
	...
	if (status == TM_BEGIN_SUCCESS) {
		...
		_lookup_verify(place, key, 0);
		ret = (place->key == key);

		TX_END(0);
	} else {
		tdata->tx_aborts++;
		tdata->tx_aborts_lookup++;
...
try_from_scratch:
	...
	if (status == TM_BEGIN_SUCCESS) {
		...
		/* _lookup_verify() will abort if verification fails. */
		_lookup_verify(place, new_node->key, 1);
		ret = _insert(rbt, place, new_node);
