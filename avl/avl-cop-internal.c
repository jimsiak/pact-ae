...
static avl_node_t *avl_node_new(int key, void *data)
{
	...
	node->key = key;
	node->data = data;
	node->height = 1; // new nodes have height 1 and NULL has height 0.
	node->live = 0;
	node->parent = NULL;
...
static inline void _lookup_verify(avl_t *avl, avl_node_t *node, int key)
{
	if (!node) {
		if (!avl->root)
			return;
...
static inline void _insert_verify(avl_t *avl, avl_node_t *node, int key)
{
	_lookup_verify(avl, node, key);
...
static inline void _delete_verify(avl_t *avl, avl_node_t *node, int key)
{
	_lookup_verify(avl, node, key);
}
...
static inline int _insert(avl_t *avl, avl_node_t *new_node, avl_node_t *place)
{
	...
	_avl_insert_fixup(avl, new_node->key, new_node->parent);
	return 1;
}
...
static int _avl_insert_warmup_helper(avl_t *avl, int key, void *value)
{
	...
	place = _traverse(avl, key);
	int ret = _insert(avl, new_node, place);
	if (!ret)
		free(new_node);
	return ret;
}
...
static inline int _delete(avl_t *avl, int key, avl_node_t *z)
{
	...
	_avl_delete_fixup(avl, y->key, y_parent);
